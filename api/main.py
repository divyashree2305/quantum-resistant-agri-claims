...
class HandshakeRequest(BaseModel):
    ...
    client_public_key: Optional[str] = Field(None, description="Client's Kyber public key (base64 encoded). If not provided, server will generate one.")


class HandshakeResponse(BaseModel):
    """Response model for Kyber handshake"""
    server_public_key: str
    client_public_key: str  # Return the client's public key (generated by server if not provided)
    ciphertext: str
    session_token: str
...
async def handshake(request: HandshakeRequest):
    ...
    """
    try:
        # Handle client public key: if provided, use it; otherwise generate server-side
        client_public_key = None
        client_key_generated_by_server = False
        
        if request.client_public_key:
            # Client provided a key - validate and use it
            try:
                ...
                try:
                    # Test if the key is valid by attempting encapsulation (we'll regenerate below)
                    _, _ = crypto.kyber_encapsulate(client_public_key)
                except Exception as e:
                    # Key format is invalid - generate a new one server-side
                    client_key_generated_by_server = True
                    client_public_key, _ = crypto.generate_kyber_keypair()
            ...
            # Client didn't provide a key - generate one server-side
            client_key_generated_by_server = True
            client_public_key, _ = crypto.generate_kyber_keypair()
...
async def get_claim_status(
    ...
    """
    Get claim status by log entry ID.
    
    Returns:
        Claim information including fraud score and tamper verification status
    """
    # Find the log entry using the helper function (it handles DB session)
    log_entry = append_log.get_log_entry_by_id(log_entry_id)
    
    if not log_entry:
        raise HTTPException(
            status_code=404,
            detail=f"Log entry {log_entry_id} not found"
        )
    
    # Try to verify AI score which contains fraud score and model version
    fraud_score = None
    model_version = None
    
    try:
        ai_score_result = verify_ai_score_func(log_entry_id)
        if ai_score_result.get("verified", False):
            fraud_score = ai_score_result.get("fraud_score")
            model_version = ai_score_result.get("model_version")
    except Exception:
        # If verification fails, continue without fraud score
        pass
    
    # Verify the log chain integrity
    tamper_verified = False
    try:
        from verify_log import verify_log_chain
        tamper_verified = verify_log_chain([log_entry])
    except Exception:
        tamper_verified = False
    
    return ClaimStatusResponse(
        log_entry_id=log_entry.id,
        claim_id=log_entry.claim_id,
        event_type=log_entry.event_type,
        timestamp=log_entry.timestamp_local,
